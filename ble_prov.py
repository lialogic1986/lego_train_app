...
class BleProvisioner:
    ...
    async def _provision_one(self, address: str, train_name: str, cfg: ProvisioningConfig) -> bool:
        ...
        async with BleakClient(address, timeout=self.connect_timeout_s) as client:
            ...
            # COMMIT
            try:
                await client.write_gatt_char(UUID_CMD, CMD_COMMIT, response=True)
                self._log(f"[{train_name}] COMMIT sent")
            except Exception as e:
                self._log(f"[{train_name}] COMMIT write failed: {e}")
                return False
